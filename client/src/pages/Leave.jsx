...
import { CheckCircleIcon, ClockIcon, PalmtreeIcon, PlusIcon, ThermometerIcon, UmbrellaIcon, XCircleIcon } from "lucide-react"
...
const Leave = () => {
  ...
  const leaveStats = isAdmin
    ? [
      {label: "Pending Requests", value: pendingCount, icon: ClockIcon},
      {label: "Approved Leaves", value: approvedCount, icon: CheckCircleIcon},
      {label: "Rejected Leaves", value: rejectedCount, icon: XCircleIcon},
    ]
    : [
            ...
            <div key={s.label} className="card card-hover p-5 sm:p-6 flex items-center gap-4 relative overflow-hidden group">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900 tracking-tight">
                  {s.value} <span className="text-sm font-normal text-slate-400">{isAdmin ? "total" : "taken"}</span>
                </p>
              </div>
            </div>
          ))}
